...
| `scripts/tmb.sh`, `scripts/tmb.ps1` | Cross-platform lifecycle/menu/update/backup/cleanup command with requested/candidate withdrawal guards before state changes; Linux adds prepared-image/read-only-data static preflight, pre-downtime image pulls, exact writer/service-state tracking, private atomic offline backups, isolated transactional replacement, offline post-install plus conditional post-start online verification, runtime probes, SIGINT recovery, rollback, `tmb` repair, and guarded project-image reclamation. The Linux entrypoint only resolves paths, sources the shared libraries, dispatches commands, and hosts the interactive menus |
| `scripts/lib/common.sh` | Shared helpers: path resolution, TTY/confirmation, `flock`/mkdir management lock, central secret redaction (`sanitize_stream`), size helpers |
| `scripts/lib/ui.sh` | Interactive menu/color helpers; the menu listing goes to stderr so command substitution captures only the selection |
| `scripts/lib/services.sh` | Compose wrapper, exact service-state tracking, health verification with crash/restart detection |
| `scripts/lib/update.sh` | Transactional verified updater (checksum, blocked releases, pre-download, candidate validation, writer stop, consistent backup, rollback snapshot, offline/online doctors, SIGINT recovery, exact state restoration) |
| `scripts/lib/backup.sh` | Consistent operational/migration backups: manifest + SHA-256, 0600 permissions, list/inspect/verify/secure/delete; Python-JSON manifest parsing; pipefail-safe member checks; path/secret redaction in inspection |
| `scripts/lib/restore.sh` | Transactional restore and migration import: archive validation (traversal/symlink rejection), staged-state validation, rollback snapshot, restored-runtime-identity ownership repair for the private config, automatic rollback (owner/mode/contents) on any failure or SIGINT |
| `scripts/lib/status.sh` | Dashboard: versions, digest, service state/health/restarts, disk usage, sizes, Telegram/logger/channels/Local API state — all derived from the typed application CLI on the compose network |
| `scripts/lib/storage.sh` | Storage overview and bounded project-scoped cleanup with strong confirmations |
| `scripts/lib/docker.sh` | Project-scoped Docker helpers: image/container views, pull, cleanup preview, old-image reclamation |
| `scripts/lib/logs.sh` | Log browsing (`--tail`/`--since`/errors/follow) and the sanitized diagnostic support bundle |
| `scripts/lib/telegram.sh` | Telegram token/admin/support/polling wizards, required-channel management, cookies status, logger flags (frontends over the typed application CLI) |
| `scripts/lib/diagnostics.sh` | Doctor frontends and SQLite/queue/Redis health |
| `scripts/lib/config.sh` | Configuration frontend over `telegram-media-bot config-edit` (typed, atomic, rollback copy) |
...
| `scripts/tests/` | Mocked withdrawal/recovery plus opt-in privileged release-upgrade tests for direct/candidate version denial, allowed forward recovery, phased offline/online verification, exact mixed service restoration, active Local API logs, archive inclusion/exclusion, redacted failures, SIGINT, runtime identity, filesystem/SQLite permissions, Compose bind contracts, the delayed Local API startup readiness regression (`test_local_api_readiness.sh`), and the real-container Local Bot API health/zero-restart regression (`test_local_api_container_health.sh`). `test_tmb.sh` is the deterministic control-plane suite: dispatch/help/usage errors, no-TTY menus, status, services, log option parsing, storage scoping, backup manifest/checksum/permissions, archive attack rejection (traversal/symlink), transactional restore with rollback and interrupt recovery, exact service-state restoration, the management lock, migration export/import, the sanitized support bundle, redaction (including token-in-path archive entries), JSON manifest values, large-archive pipefail verification, backup secure/warn, and safe uninstall stages — all through a stateful fake docker so no daemon/root/network is required |
