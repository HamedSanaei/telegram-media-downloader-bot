...
| `scripts/lib/backup.sh` | Consistent operational/migration backups: manifest + SHA-256, 0600 permissions, list/inspect/verify/delete |
| `scripts/lib/restore.sh` | Transactional restore and migration import: archive validation (traversal/symlink rejection), staged-state validation, rollback snapshot, automatic rollback on any failure or SIGINT |
| `scripts/lib/status.sh` | Dashboard: versions, digest, service state/health/restarts, disk usage, sizes, Telegram/logger/channels/Local API state |
