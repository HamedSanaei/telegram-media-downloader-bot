...
#   pre-restore backup -> extract to staging -> validate staged state ->
#   swap persistent entries (rollback snapshot) -> repair permissions ->
#   config-check + offline doctor -> restore exact service state ->
#   online verification. Any failure rolls back automatically. The mutating
#   transaction runs in a subshell so its SIGINT trap can never leak.
# --------------------------------------------------------------------------- #
